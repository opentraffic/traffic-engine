...
public class SpatialDataStore {
	...
	public static Fun.Tuple2<Integer, Integer> getTile(final double lat, final double lon) {
	...
	public static List<Fun.Tuple2<Integer, Integer>> getInternalTilesForZ(final int x, final int y, int z, int internalZ) {
		...
		for(int xOffset = 0; xOffset < zScale; xOffset++) {
			for(int yOffset = 0; yOffset < zScale; yOffset++) {
				tiles.add(new Fun.Tuple2<>(newTileX + xOffset, newTileY + yOffset));
			}
		}
